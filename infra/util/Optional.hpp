...
//  this class, you can delay construction of an object, or hasten destruction.
...
//  opt = infra::none;              // Reset the optional
//  if (!opt)
...
namespace infra
{
    ...
    Optional<T>::Optional(Optional&& other) noexcept
    {
        if (other.storage.has_value())
        {
            storage.emplace(std::move(*other.storage));
            other.storage.reset();
        }
    }
    ...
    Optional<T>::Optional(const Optional<U>& other)
        : storage(other ? std::optional<T>(*other) : std::optional<T>())
    {}
    ...
    {
        return storage.value_or(std::forward<T>(value));
    }

    template<class T>
    const T& Optional<T>::ValueOr(const T& value) const
    {
        return storage.has_value() ? *storage : value;
    }

    template<class T>
    T Optional<T>::ValueOrDefault() const
    {
        return storage.value_or(T{});
    }
    ...
    {
        return storage.emplace(list, std::forward<Args>(args)...);
    }
