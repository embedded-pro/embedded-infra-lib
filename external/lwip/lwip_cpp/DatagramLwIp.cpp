...
namespace services
{
    ...
    DatagramExchangeLwIP::UdpWriter::UdpWriter(udp_pcb* control, pbuf* buffer, const std::optional<UdpSocket>& remote)
        : control(control)
        , buffer(buffer)
        , remote(remote)
    {}
    ...
    DatagramExchangeLwIP::StateWaitingForBuffer::StateWaitingForBuffer(DatagramExchangeLwIP& datagramExchange, std::size_t sendSize, const std::optional<UdpSocket>& remote)
        : datagramExchange(datagramExchange)
        , sendSize(sendSize)
        , remote(remote)
