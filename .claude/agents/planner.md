---
name: planner
description: Use when a detailed implementation plan is needed before writing code. Produces structured, actionable plans that follow all EmIL embedded C++ constraints, SOLID principles, and project conventions. Best for complex features, architectural changes, or multi-file modifications. Does NOT write or edit code.
...
- **Documentation**: Consult `docs/` for domain-specific guidance:
  ...
  - `docs/Echo.md` — ECHO RPC, protobuf conventions, service/method IDs
  - `docs/Sesame.md` — SESAME serial protocol layers
...
- Configure command: `cmake --preset host`
- Build command: `cmake --build --preset host-Debug`
- Test command: `ctest --preset host`
...
- [ ] **WeakPtr safety**: Classes that schedule via `EventDispatcherWithWeakPtr::Instance().Schedule()` must use an `infra::WeakPtr<T>` target so the action is automatically skipped if the object is expired
...
- [ ] Acronyms as words: `Uart` not `UART`, `Spi` not `SPI`
