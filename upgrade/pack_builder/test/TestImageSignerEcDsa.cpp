...
class TestImageSignerEcDsa224
    : public testing::Test
{
public:
    TestImageSignerEcDsa224()
...
TEST_F(TestImageSignerEcDsa224, should_report_algorithm_details)
{
    EXPECT_EQ(1, signer.SignatureMethod());
    EXPECT_EQ(56, signer.SignatureLength());
}

TEST_F(TestImageSignerEcDsa224, should_sign_image)
{
...
class TestImageSignerEcDsa256
    : public testing::Test
{
public:
    TestImageSignerEcDsa256()
        : randomDataGenerator(std::vector<uint8_t>(1000, 5))
        , signer(randomDataGenerator, ecDsa256PublicKey, ecDsa256PrivateKey)
    {}

    hal::SynchronousFixedRandomDataGenerator randomDataGenerator;
    application::ImageSignerEcDsa256 signer;
    ...
    const std::array<uint8_t, 64> ecDsa256PrivateKey = { { 0xff, 0xd1, 0x3b, 0x3b, 0x7f, 0xb7, 0xf0, 0xd4,
...
TEST_F(TestImageSignerEcDsa256, should_report_algorithm_details)
{
    EXPECT_EQ(2, signer.SignatureMethod());
    EXPECT_EQ(64, signer.SignatureLength());
}

TEST_F(TestImageSignerEcDsa256, should_sign_image)
{
    std::vector<uint8_t> signature = signer.ImageSignature(std::vector<uint8_t>{ 0, 1, 2, 3 });
