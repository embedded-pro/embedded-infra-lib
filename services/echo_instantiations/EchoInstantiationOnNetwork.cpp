        ...
        else if (services::SchemeFromUrl(target) == "ws")
        {
            if (!infra::host::WaitUntilDone(
                    [&](const std::function<void()>& done)
                    {
                        ...
                        auto echoClient = std::make_shared<EchoClientWebSocket>(connectionFactory, randomDataGenerator, target, services::PortFromUrl(target).value_or(80));
        ...
        else if (services::SchemeFromUrl(target) == "ws")
        {
            if (!infra::host::WaitUntilDone(
                    [&](const std::function<void()>& done)
                    {
                        ...
                        auto echoClient = std::make_shared<TracingEchoClientWebSocket>(connectionFactory, randomDataGenerator, target, services::PortFromUrl(target).value_or(80), tracer);
