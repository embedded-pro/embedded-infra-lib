...
TEST_F(RetryGattClientCharacteristicsOperationsTest, should_call_read_characteristic)
{
    ...
    EXPECT_CALL(connection, Read(handle, testing::Ref(onReadMock), testing::Ref(onDoneMock)));

    retryAdapter.Read(handle, onReadMock, onDoneMock);
}

TEST_F(RetryGattClientCharacteristicsOperationsTest, should_call_write_characteristic)
{
    EXPECT_CALL(connection, Write(handle, testing::ElementsAreArray(data), testing::Ref(onDoneMock)));

    retryAdapter.Write(handle, data, onDoneMock);
}
...
TEST_F(RetryGattClientCharacteristicsOperationsTest, should_call_write_without_response_characteristic_and_error)
{
    ...
    EXPECT_CALL(connection, WriteWithoutResponse(handle, testing::ElementsAreArray(data), testing::_))
...
{
    EXPECT_CALL(connection, EnableNotification(handle, testing::Ref(onDoneMock)));
...
{
    EXPECT_CALL(connection, DisableNotification(handle, testing::Ref(onDoneMock)));
...
{
    EXPECT_CALL(connection, EnableIndication(handle, testing::Ref(onDoneMock)));
...
{
    EXPECT_CALL(connection, DisableIndication(handle, testing::Ref(onDoneMock)));
